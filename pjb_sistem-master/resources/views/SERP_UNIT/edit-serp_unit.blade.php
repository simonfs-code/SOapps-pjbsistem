...
@section('container')
    ...
    <div class ="card card-info card-outline col-8">
        ...
        <div class="card-body">
          <form action="/update-serp_unit/{{$serp_unit->serp_unit_id}}" method="post">
          @method('patch')
          ...
          <div class="row">
            ...
            <label>Blok </label>
              ...
              @foreach($blok as $data)
                <option value="{{$data->serp_blok_id}}" {{($serp_unit->serp_blok_id == $data->serp_blok_id) ? 'selected':''}}>{{$data->serp_blok_name}}</option>
               @endforeach
              </select>
            ...
            <div class="form-group col-sm-7">
            <label>Unit</label>
              <input type="text" name="serp_unit_name" class="form-control @error('serp_unit_name') is-invalid @enderror" value="{{$serp_unit->serp_unit_name}}">
